...
var RawParserModule = /*#__PURE__*/function () {
  ...
  _createClass(RawParserModule, [{
    ...
    value: function subBuf(startOpen, endOpen) {
      // sub block [m,n]
      // nal
      var returnBuf = new Uint8Array(this.stream.subarray(startOpen, endOpen + 1));
    ...
    value: function nextNalu() {
      ...
      for (var i = 0; i < this.stream.length; i++) {
        ...
        if (
        // 0x00 00 01
        this.stream[i] == 0 && this.stream[i + 1] == 0 && this.stream[i + 2] == 1 ||
        // 0x00 00 00 01
...
onmessage = function onmessage(event) {
  ...
  switch (cmd) {
    ...
    case 'get-nalu':
      ...
      var nalBuf = g_RawParser.nextFrame();
